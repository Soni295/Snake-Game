...
type iReducer = (snake: character, action: {scale: number, type: string}) =>
  character
...
export const reducer: iReducer = (snake, action) => {
  ...
  interface iCases { [x: string]: (direction: string) => character }
  const cases: iCases = {
  ...
  const setMove = {
    [actionTypes.ARROWLEFT] : () => head.x === neck.x + action.scale
      ? snake.direction
      : actionTypes.ARROWLEFT,
    [actionTypes.ARROWRIGHT] : () => head.x === neck.x - action.scale
      ? snake.direction
      : actionTypes.ARROWRIGHT,
    [actionTypes.ARROWDOWN] : () => head.y === neck.y - action.scale
      ? snake.direction
      : actionTypes.ARROWDOWN,
    [actionTypes.ARROWUP] : () => head.y === neck.y + action.scale
      ? snake.direction
      : actionTypes.ARROWUP
  }
